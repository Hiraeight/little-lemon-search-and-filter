...
export function saveMenuItems(menuItems) {
  db.transaction((tx) => {
    ...
    menuItems.forEach((item) => {
      // For UUID, you can use a simple strategy like generating a random value, or use a library like uuid
      const uuid = `menu-${item.id}`;
      tx.executeSql(insertSQL, [item.id, uuid, item.title, item.price, item.category]);
    });
  });
}
...
export async function filterByQueryAndCategories(query, activeCategories) {
  return new Promise((resolve, reject) => {
    // Build the SQL query with conditions for both query and categories
    const queryParts = [];
    const params = [];
    ...
    if (query) {
      ...
      params.push(`%${query}%`);  // Using % to represent wildcard characters for substring matching
    }

    // Add condition for categories if provided
    if (activeCategories.length > 0) {
      queryParts.push('category IN (' + activeCategories.map(() => '?').join(', ') + ')');
      params.push(...activeCategories);
    }

    const sql = 'SELECT * FROM menuitems WHERE ' + queryParts.join(' AND ');
